...
import { useScrollPosition } from '@landing/hooks/useScrollPosition';
...
export default function Header() {
  ...
  const navLinks = [
    ...
    { to: '/sentimen-analisis', label: t('nav.sentimen') },
    ...
    >
      <nav className="container-wide flex items-center justify-between h-16 md:h-20">
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <Link
            to="/login"
            className="font-body text-sm font-medium px-4 py-2 rounded-lg border border-white/10 hover:bg-ifrit-red/5 transition-all text-text-on-dark-muted hover:text-ifrit-red hover:border-ifrit-red/30"
          >
            {t('nav.login')}
          </Link>
