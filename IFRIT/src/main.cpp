...
// Wokwi simulation uses DHT22 for temp/humidity, but we map it to SHTC3 backend mapping
#include "DHTesp.h"
DHTesp dht;
...
void loop() {
  ...
  if (currentMillis - lastTelemetryTime >= TELEMETRY_INTERVAL_MS) {
    ...
    int httpCode = http.POST(payload);
    if (httpCode > 0 && httpCode < 400) {
      Serial.println("Telemetry sent OK. (" + String(httpCode) + ") Alarm: " + (alarmTriggered ? "ON" : "OFF"));
    } else {
      Serial.println("Telemetry failed: " + String(httpCode) + " - " + http.getString());
    }
